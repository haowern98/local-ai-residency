...
ONNX sessions can be listed, loaded, chatted with, saved after context exists,
evicted, and restored by the CLI:
...
decoding. The CLI prints decoded text, not raw token IDs.
...
Chat generation options are read from `sessions.txt`. If omitted, the llama.cpp
defaults are `max_tokens=512`, `temp=0.8`, `min_p=0.05`, and the llama.cpp
default seed. ONNX chat currently uses greedy next-token selection with
`max_tokens=512` by default.
For llama.cpp sessions, MosaicVRAM first tries the GGUF chat template, then
falls back to supported built-in templates. You can override detection with an
explicit template name, for example `chat_template=gemma`. If a model emits
visible turn markers, set `stop_strings` to a comma-separated list, for example
