...
std::string OptionalEnvironmentString(const char* name) {
#ifdef _WIN32
  char* value = nullptr;
  std::size_t size = 0;
...
std::vector<int64_t> RequiredOnnxPromptTokens(const PlanLine& line) {
  ...
  const auto tokenizer = line.values.find("tokenizer");
  ...
  if (tokenizer == line.values.end() || tokenizer->second.empty()) {
    std::ostringstream message;
    message << "line " << line.line_number
            << " missing required key for ONNX prompt tokenization: tokenizer";
    throw std::runtime_error(message.str());
  }
  ...
  if (prompt != line.values.end()) {
    ...
    prompt_text =
        ReadRequiredTextFile(prompt_file->second, line.line_number,
                             "prompt_file");
  }
  HfTokenizerOptions tokenizer_options;
  tokenizer_options.tokenizer_path = tokenizer->second;
  const std::string python_from_env =
      OptionalEnvironmentString("MOSAICVRAM_PYTHON");
  if (!python_from_env.empty()) {
    tokenizer_options.python_executable = python_from_env;
  }
  tokenizer_options.python_executable =
      OptionalString(line, "tokenizer_python",
                     tokenizer_options.python_executable);
  return TokenizeWithHfTokenizer(tokenizer_options, prompt_text);
}
...
std::vector<int64_t> OnnxContinuationTokens(const PlanLine& line,
  ...
  HfTokenizerOptions tokenizer_options;
  tokenizer_options.tokenizer_path = session.tokenizer_path;
  if (!session.tokenizer_python.empty()) {
    tokenizer_options.python_executable = session.tokenizer_python;
  }
  return TokenizeWithHfTokenizer(tokenizer_options, text->second);
}
#endif  // MOSAICVRAM_ENABLE_ONNX
...
int RunPlan(const Options& options) {
  ...
  for (const PlanLine& line : lines) {
    ...
    session.tokenizer_python = OptionalString(line, "tokenizer_python", "");
